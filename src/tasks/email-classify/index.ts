...
export const EmailClassifyJob = Job.builder()
  ...
  .description(
    'The Email Classifier automatically processes and categorizes incoming emails',
  )
  ...
  .step('Initial Classification', async context => {
    await wait(1000);
    context.logger(
      'Automatically classifying emails using predefined rules or algorithms.',
    );
    await wait(1000);
    context.logger(
      '1 labeled as Business, 1 labeled as Promotions, 1 Uncategorized.',
    );
  })
  ...
  .step('Notion Update', context => {
    ...
    context.logger(
      'Adding or updating classified email data in the Notion table.',
    );
    return wait(3000).then(() => context.payload);
  })
