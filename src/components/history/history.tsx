...
import { mdiCalendar, mdiCheck, mdiChevronDown, mdiClockTimeEightOutline } from '@mdi/js'
      ...
      .flatMap(step => {
        ...
        lines.push(
          <p className='px-6 flex items-center'>
            {`Starting...`} <span className='ml-2 text-blue-500'>{step.name}</span>
          </p>,
        )
        ...
        lines.push(
          ...step.logs
            .flatMap(line => {
              return isObject(line) ? JSON.stringify(line, null, 2).split('\n') : line
            })
        ...
        if (step.error) {
          if (step.error.stack) {
            step.error.stack.split('\n').forEach(line => {
              lines.push(<p className='px-6 bg-red-400/20 text-red-400'>{line}</p>)
            })
          } else {
            lines.push(
          ...
          lines.push(
            <p className='flex items-center px-6 mb-4'>
              <Icon path={mdiCheck} className='mr-2 text-green-400' size={0.6} />
              {`finished at ${dayjs(step.finishedAt).format('YYYY-MM-DDTHH:mm')}`}
            </p>,
          )
        }
          ...
          <div className='text-xs mx-8'>
            ...
            <div className='flex items-center mt-1'>
              ...
              <span className='ml-0.5'>
                {formatDuration(Date.now() - (history.finishedAt || history.startedAt!))} ago
              </span>
            </div>
          </div>
          <div
      ...
      {select && (
        <div className='bg-white/5 border-t border-x grid grid-cols-1 md:grid-cols-2 md:max-h-[450px] overflow-y-auto'>
              ...
              {history.thread.map(step => (
                <div className={`-translate-x-[9px]`} key={step.name}>
                  <div className='flex items-center select-none py-6'>
                    <div
                      className={`${ColorWithJob(history.status).BackgroundColorByStatus(
                        step.status,
                      )} ${ColorWithJob(history.status).TextColorByStatus(
                        step.status,
                      )} ${ColorWithJob(history.status).RingColorByStatus(
                        step.status,
                      )} ring w-5 h-5 rounded-full mr-2 p-[2px]`}
                    >
                      <Icon path={IconByStatus(step.status)} />
                    </div>
                    <h1 className={`${status != 'READY' ? 'text-white' : ''} text-base font-bold`}>
                      {step.name}
                    </h1>
                    <span className='text-xs ml-auto mr-4'>{formatDuration(step.duration)}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className='w-full border-l-0 md:border-l border-t md:border-t-0 h-full relative'>
            <div className={`min-h-20 text-neutral-500 py-8 text-xs`}>{flatLogs}</div>
          </div>
        </div>
      )}
    </div>
